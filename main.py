...
async def send_custom_pings(ws):
    while True:
        ping_payload = {
            "id": "ping",
            "payload": {
        ...
        message = "422" + json.dumps(["cmd", ping_payload])
        try:
            await ws.send(message)
        except Exception as e:
            break
            print("[send_custom_pings] Erro ao enviar ping:", e)
...
async def run_bot_cycle():
    ...
    async with connect(uri, ping_interval=20, ping_timeout=20) as ws:
        ...
        asyncio.create_task(send_custom_pings(ws))
